...
int main () {
    // Variables to store fare, distance, vehicle choice, and rush hour status
    double fare;
    double distance;
    int choice;
    int rushhour;
    ...
    if (distance <=0){
        ...
        // Calculate base fare depending on vehicle type
        switch (choice)
        {
    ...
    // If rush hour, apply 25% surcharge and then discount rules
    if (rushhour == 1)
    { 
        switch (choice)
        {
    ...
    // If not rush hour, apply discount rules directly
    { 
        switch (choice)
        {
        case 1:
